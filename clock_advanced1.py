...
class Alarm:
    ...
    def sound(self):
        stamp = time.time()
        if stamp - self.last_ring < SOUND_INTERVAL:
            return
        self.last_ring = stamp
        if play_sound():
            return
        try:
    ...
    def refresh(self, theme, theme_name):
        ...
        if self.ringing:
            ...
            source = os.path.basename(SOUND_FILE) if os.path.exists(SOUND_FILE) else "system bell"
            text = f"A: set alarm   C: clear   T: theme [{theme_name}]   P: preview [{source}]"
            if self.target:
